...
`examples/` 有完整示例研报（AI 服务器电源瓶颈）；`examples/cases/` 有几份真实事件/主题研报（覆铜板涨价、AI 电力、InP 出口管制、先进封装材料）。这些 PDF 是 **v2 skill 生成的参考产物**，仅用于展示任务形态、版式层级和交付颗粒度；v3 在此基础上重写了触发规则、证据闸门、目标价字段和交付 QA。

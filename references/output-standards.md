...
- 不出现缺字方块或无法复制的乱码。
- 中文 PDF 默认只接受稳定可嵌入的 TrueType/TTC CJK 字体；不得把 Noto/思源 OTF/CFF 字体当作“内置字体修复”。若 WeasyPrint 找不到安全 TTF/TTC，必须主动回退 ReportLab，并用页面 PNG 预览确认中文没有掉字。
- 不出现内部流程痕迹：`Graph Gate`、`edge 记录`、`edges.json`、`source, target`、`source、target`、`relationship`、`evidence_level`、`bypass_risk`。
- `作者：瓶颈侦察 v3` 只低调出现在报告元信息，不重复污染页眉页脚。
